...
% function [P, error] = bestLeastSquaresFit(X, Y, degree, verbose)
%     if nargin < 4  % Check if verbose argument is provided
%         verbose = true;  % Default value of verbose is true
%     end
% 
...
%         error('X and Y vectors must be of the same length.');
%     elseif verbose
%         disp('Validation complete: X and Y vectors are of the same length.');
%     end
% 
%     % Prepare the Vandermonde matrix for the given degree
%     V = vander(X);
%     V = V(:, end-degree:end);
%     if verbose
%         disp('Vandermonde matrix V prepared for the given degree:');
%         disp(V);
%     end
% 
%     % Solve for polynomial coefficients using the backslash operator
%     P = V \ Y;
%     if verbose
%         disp('Solving for polynomial coefficients P using the equation V\\P = Y:');
%         disp(['V = ', mat2str(V, 3)]);
%         disp(['Y = ', mat2str(Y', 3)]);
%         disp(['P (Coefficients) = ', mat2str(P', 3)]);
%     end
% 
%     % Calculate the error of the fit
%     Y_fit = V * P; % Calculate fitted Y values
%     error = norm(Y - Y_fit); % Euclidean norm of residuals
%     if verbose
%         disp('Calculating the error of the fit using norm(Y - V*P):');
%         disp(['Y_fit = V*P = ', mat2str(Y_fit', 3)]);
%         disp(['Y = ', mat2str(Y', 3)]);
%         disp(['Error of the fit = ', num2str(error)]);
%     end
% 
%     % Display the polynomial coefficients and error if verbose is true
%     if verbose
%         disp('Polynomial Coefficients (from highest to lowest degree):');
%         disp(P');
%     end
% end
